...
class Validation
{
    ...
    {
        $this->logger = ApplicationContext::getContainer()->get(LoggerFactory::class)->get('validation');
    }
        ...
        foreach ($rules as $field => $rule) {
            ...
            foreach ($constraints as $constraint) {
                ...
                if (method_exists($this, $func_rule)) {
                    ...
                    $this->logger->info(
                        sprintf('validation key:%s rule:%s result:%s',
                        $field_name,
                        $func_rule,
                        $check ? 'true' : 'false')
                    );
                ...
                if (method_exists($this, $func_filter)) {
                    ...
                    $this->logger->info(
                        sprintf('validation key:%s filter:%s result:%s',
                            $field_name,
                            $func_filter,
                            $filter_value)
                    );
                ...
                if (strpos($func, 'cb_') !== false && method_exists($obj, $customMethod)) {
                    ...
                    $this->logger->info(
                        sprintf('validation key:%s cb:%s result:%s',
                            $field_name,
                            $customMethod,
